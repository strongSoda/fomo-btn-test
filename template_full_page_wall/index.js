...
sortReviews(REVIEWS).then(ascending_reviews => {
    appendFomoWall(createFomoWall(ascending_reviews.slice(reviews_index, 4)))
    reviews_index += 4
})
...
LOAD_MORE_REVIEWS.addEventListener('click', () => {
    ...
    if(reviews_index < REVIEWS.length) {
        // create fomo wall
        sortReviews(REVIEWS).then(ascending_reviews => {
            appendFomoWall(createFomoWall(ascending_reviews.slice(reviews_index, reviews_index + 4)))
            reviews_index += 4
        })
    } else {
